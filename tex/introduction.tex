...
using data from a Navy/HYCOM, 3D-Var-based reanalysis product
 as ``Truth'' (upper row)
\red{CITE} (see \cref{subsec:datasets} for details).
We generate the prediction (middle row) with a RNN architecture
described more fully in \cref{subsubsec:rc-architecture}.
...
in \cref{subsec:rnn-architecture}).
Throughout the study, we focus our attention on how well these RNNs can emulate Surface
Quasi-Geostrophic (SQG) motion (\cref{subsec:datasets}).
We show in \cref{sec:sqg} that this relatively idealized model exhibits similar
