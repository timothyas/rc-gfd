...
etc.).
Due to the high computational cost of the physical model that underlies the
forecasting system, obtaining reliable forecasts typically results in
...
reality.
As a result, reliable predictions require
statistical methods like ensemble based forecasting in order to represent this
uncertainty.
Producing an ensemble with reliable statistical significance
...
using data from a Navy/HYCOM, 3D-Var-based reanalysis product
 as ``Truth'' (upper row; seee \cref{sec:datasets} for data details).
...
predictions.
Our primary goal is to explore at least some reasons why this occurs
and offer one technique to mitigate its effects.
...
In order to isolate this effect, we do not rely on the GoM reanalysis data.
Instead, we use a model for Surface Quasi-Geostrophic (SQG) turbulence because
we observe similar behavior and while avoiding complexities related to
multivariate interactions and the specifics of a data assimilation system.
Additionally, using the model equations directly allows us to more directly
control the temporal resolution of the data used for training, validation, and
testing.
The SQG model and dataset generation is described more fully in
\cref{subsec:sqg}.

The architectures that we use in this study broadly stem from a class of machine
learning termed as reservoir computing (RC).
...
repeatedly shown that it can emulate low dimensional chaotic systems and achieve
excellent prediction skill
\citep<e.g.>[]{platt_systematic_2022,griffith_forecasting_2019,lu_attractor_2018,pathak_model-free_2018},
such that it can be competitive with more complex RNNs like LSTMs
\citep{vlachas_backpropagation_2020}.
...
Additionally, \citep{penny_integrating_2022} showed that RC can be
