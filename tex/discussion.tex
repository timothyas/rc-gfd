...
Spectral bias at high wavenumbers is a known phenomenon
...
Here, we showed that this bias arises in NVAR and ESN architectures in relation
...
The CFL condition is therefore a barrier to weather and climate models.
...
here we see that increasing the timestep causes a sort of numerical dissipation,
which can actually stabilize an otherwise unstable model architecture
(\cref{subsec:nvar-subsampling}).
We conjecture that our observed difference between timestepping in numerical
PDEs and neural networks is similar to the principle described by
\citet{xu_overview_2022},
who show that typical solvers used in PDEs resolve small spatial scales first,
...
Earth's atmosphere, and, most importantly, they are constrained to observational
data.
However, we note reanalysis products are imperfect for at least the following reasons:
...
Given that we can qualitatively observe some degree of spectral error in
a wide variety of neural network architectures
\citep<e.g.>[]{bi_pangu-weather_2022,pathak_fourcastnet_2022,keisler_forecasting_2022},
we suggest that this issue could be more general to other neural network
architectures.
We suggest that future work should be directed at understand the degree to which
temporal resolution affects architectures other than recurrent neural networks.
Potential avenues to better understand and reliably control this phenomenon
could include exploring how attention mechanisms
\citep{vaswani_attention_2017,dosovitskiy_image_2021}
handle this temporal resolution spectral bias.
Additionally, in light of our results indicating that wider networks can at
least mitigate
the spectral bias to some degree (\cref{subsec:esn-size}), it would be
