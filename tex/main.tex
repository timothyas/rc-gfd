...
% /begin rnn.sty
\usepackage{bm}
\usepackage{bbm}        % for 1 vector
\usepackage{amsmath}    % to declare argmin
\usepackage{mathtools}  % for coloneqq
...
\newcommand{\nvarparams}{\hyperparameters_{NVAR}}
...
\citep{pathak_fourcastnet_2022,bi_accurate_2023,lam_graphcast_2022},
which is the same
...
\begin{figure}
    \centering
    \includegraphics[width=.8\textwidth]{rc_gom_sst.pdf}
...
\citep<>[Figure 3]{bi_accurate_2023},
...
\citep<e.g.>[]{lam_graphcast_2022,bi_accurate_2023,pathak_fourcastnet_2022,keisler_forecasting_2022,weyn_sub-seasonal_2021,arcomano_machine_2020}.
...
training
\citep<e.g.,>[]{lam_graphcast_2022,bi_accurate_2023,pathak_fourcastnet_2022,keisler_forecasting_2022},
...
While additional testing shows that a periodic sine activation function can
reduce the high frequency bias in KE\_NRMSE, following work by
\citet{sitzmann_implicit_2020},
the underlying problem still remains (see additional analysis in the
Supplemental Materials).
Therefore, in order to further reduce the high frequency bias, it may be necessary to
move the spectral penalties to the micro-scale
cost function, i.e., to learn the readout matrix weights in the case of
reservoir computing.
The time stepping, spectral loss, and ``small scale corrector
...
The authors thank three anonymous reviewers and the handling editor for comments
