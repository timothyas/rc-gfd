...
We employ the following RC structure
\begin{linenomath*}\begin{equation}
    \begin{aligned}
        \localhidden(n+1)
        &=
        ...
        +
        \leak \tanh\left(
            \adjacency \hidden(n) + \inputmatrix \inputstate(n) + \biasvector
            \right)
             \\
        \localoutput(n+1)
        &= \localWout \localhidden(n+1) \, .
...
and the bias vector $\biasvector\in\mathbb{R}^{\nhidden}$ with elements
...
First, the adjacency matrix is initialized with elements
$a_{m,n}\sim\mathcal{U}(-1,1)$,
normalized by its spectral radius, and scaled by the desired spectral radius
$\spectralradius$.
Secondly, the input matrix is initialized with elements
$w_{m,n}\sim\mathcal{U}(-\sigma,\sigma)$.
...
See \red{TABLE} for tabulated parameters.\todo{Fill this out once fixed}
...
%For example, \citet{chen_next_2022} showed for a polynomial-based hidden state, a
