...
bound on the timestep size that can be used in the numerical solution of PDEs.
...
RNNs suboptimal
architecture choices for model emulators in cases where representing small-scale
dynamics is important but a coarse timestep is required.
This requirement is especially true when designing a
parameterization scheme for sub-grid-scale dynamics
\citep<e.g.,>[]{perezhogin_generative_2023}, where the emulator should ideally run at the same time step as the
``large-scale'' model.
